...
$node = stac_research_node($researchKey ?? '');
...
if (!empty($node['parent'])) {
    $parentNode = stac_research_node($node['parent']);
}

$childNodes = [];
foreach ($node['children'] as $childKey) {
    $childNodes[] = stac_research_node($childKey);
}
    ...
    <section class="stac-program-page__hero" style="<?= $heroStyle ?>">
        <div class="stac-program-page__hero-inner">
            <div class="stac-program-page__hero-copy">
                <span class="stac-program-page__eyebrow"><?= htmlspecialchars($parentNode['title'] ?? 'Research', ENT_QUOTES, 'UTF-8') ?></span>
    ...
    <?php foreach (($node['sections'] ?? []) as $section): ?>
        <section class="stac-program-page__content">
            <article class="stac-program-page__card">
                <span class="stac-section-kicker"><?= htmlspecialchars($section['label'] ?? 'Research Information', ENT_QUOTES, 'UTF-8') ?></span>
