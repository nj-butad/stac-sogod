...
$node = stac_offices_node($officeKey ?? '');
...
if (!empty($node['parent'])) {
    $parentNode = stac_offices_node($node['parent']);
}

$childNodes = [];
foreach ($node['children'] as $childKey) {
    $childNodes[] = stac_offices_node($childKey);
}
...
$heroStyle = sprintf(
    "--stac-program-hero-min-height:%s;background-image:linear-gradient(135deg,rgba(5,22,56,.56) 0%%,rgba(12,42,96,.48) 52%%,rgba(23,152,218,.30) 100%%),url('%s');",
    ...
    <section class="stac-program-page__hero" style="<?= $heroStyle ?>">
        <div class="stac-program-page__hero-inner">
            <div class="stac-program-page__hero-copy">
                <span class="stac-program-page__eyebrow"><?= htmlspecialchars($parentNode['title'] ?? 'Offices', ENT_QUOTES, 'UTF-8') ?></span>
    ...
    <?php foreach ($contentSections as $section): ?>
        <section class="stac-program-page__content">
            <article class="stac-program-page__card">
                <span class="stac-section-kicker"><?= htmlspecialchars($section['label'] ?? 'Office Information', ENT_QUOTES, 'UTF-8') ?></span>
