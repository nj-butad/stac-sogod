...
if (!empty($node['parent'])) {
    $parentNode = stac_about_node($node['parent']);
}

$childNodes = [];
foreach ($node['children'] as $childKey) {
    $childNodes[] = stac_about_node($childKey);
}

$heroImage = $node['hero_image'] ?? 'upload/about-bg-1.jpg';
$heroStyle = sprintf(
    "--stac-program-hero-min-height:560px;background-image:linear-gradient(135deg,rgba(5,22,56,.54) 0%%,rgba(12,42,96,.46) 52%%,rgba(23,152,218,.28) 100%%),url('%s');background-position:center center;background-size:cover;background-repeat:no-repeat;",
    ...
    <section class="stac-program-page__hero" style="<?= $heroStyle ?>">
        <div class="stac-program-page__hero-inner">
            <div class="stac-program-page__hero-copy">
                <span class="stac-program-page__eyebrow"><?= htmlspecialchars($parentNode['title'] ?? 'About Us', ENT_QUOTES, 'UTF-8') ?></span>
    ...
    <?php foreach (($node['sections'] ?? []) as $section): ?>
        <section class="stac-program-page__content">
            <article class="stac-program-page__card">
                <span class="stac-section-kicker"><?= htmlspecialchars($section['label'] ?? 'About Information', ENT_QUOTES, 'UTF-8') ?></span>
